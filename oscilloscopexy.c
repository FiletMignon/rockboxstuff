...
static const char cfg_filename[] =  "oscilloscopexy.cfg";
...
static void osc_osd_format_message(enum osc_message_ids id, int val)
{
    ...
    case OSC_MSG_GRAPHMODE:
        msg = val == GRAPH_WAVEFORM ? "X Y" : "Peaks";
        break;
#endif
...
{
	rb->lcd_drawline(x_prev, y_prev, x, y);
	/*
                                       ...
                                       int vmin_prev, int vmax_prev)
{
	...
	*/
    (void)x_prev; (void)vmin_prev; (void)vmax_prev; (void)y_prev;
}
...
static long anim_waveform_horizontal(void)
{
	...
	*/
	int i = 2;
	while(i<(count*PCM_SAMPLESIZE)){
		int x = (1*LCD_HEIGHT/2) - WAVEFORM_SCALE_PCM(LCD_HEIGHT/2-1, waveform_buffer[i+0]);
		int y = (1*LCD_HEIGHT/2) - WAVEFORM_SCALE_PCM(LCD_HEIGHT/2-1, waveform_buffer[i+1]);
		int x_prev = (1*LCD_HEIGHT/2) - WAVEFORM_SCALE_PCM(LCD_HEIGHT/2-1, waveform_buffer[i-2]);
		int y_prev = (1*LCD_HEIGHT/2) - WAVEFORM_SCALE_PCM(LCD_HEIGHT/2-1, waveform_buffer[i-1]);
		plot[osc.draw](x, x_prev, y, y_prev, 0,0);
		i+=2;
	}
